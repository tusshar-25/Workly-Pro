  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 h-20 text-white transition-all duration-700 navbar-glow ${
        scrolled
          ? "py-1 shadow-2xl backdrop-blur-xl bg-opacity-80"
          : "py-2"
      }`}
    >
      {/* Glow underline */}
          ...
          {user ? (
            ...
            <div className="flex items-center gap-3">
              {location.pathname === "/" ? (
                <Link
                  to="/register"
                  className="px-3 py-1 bg-white text-blue-700 rounded-lg font-semibold shadow-md hover:shadow-lg hover:scale-105 transition-all"
                >
                  Register
                </Link>
              ) : (
